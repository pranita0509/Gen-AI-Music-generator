...
# ── In-memory OTP store (fixes Render multi-worker/SQLite issues) ─
_otp_store = {}   # { identifier: {'otp': '1234', 'expires': timestamp} }
...
def send_code():
    ...
    # Store in memory (survives within same worker — use --workers 1 on Render)
    _otp_store[identifier] = {'otp': otp, 'expires': time.time() + OTP_EXPIRY}
    # Also sync to DB so user row exists for later track saving
...
def verify_code():
    ...
    # Check in-memory store first (reliable across Render restarts within session)
    ...
    # Fallback to DB (in case server restarted between send and verify)
